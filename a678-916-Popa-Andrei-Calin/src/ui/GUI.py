...
class GUI:
    ...
    def start(self):
        ...
        self.sort_time_btn = Button(frame, text='Sort Date', width=18, command=self._sort_time, bg="#3C3F41",
                                    fg="#BBBBBB", font='Consolas 11')
        self.sort_time_btn.grid(row=6, column=11)
        self.sort_person_btn = Button(frame, text='Sort Person', width=18, command=self._sort_person, bg="#3C3F41",
                                      fg="#BBBBBB", font='Consolas 11')
    ...
    def _sort_person(self):
        try:
            ...
            self.list1.insert(1,
                              '----------------------------------------------------------------------------------------------------------------------')
            person_list = self.service.a_person(self.sort_person.get())
            if person_list and person_list[0] == ' ':
                raise ValueError(person_list)
            while person_list:
                self.list1.insert(END, person_list[:person_list.find('\n')])
