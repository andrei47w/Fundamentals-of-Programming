...
class Service:
    ...
    def p_remove(self, id):
        ...
        for person in self.repo._persons:
            if person.pers_id == id:
                ...
                self.history_add(['self.repo.remove({})'.format(id),
                                  "self.repo.p_add({}, '{}', '{}')".format(person.pers_id, person.name,
                                                                           person.phone_number)])
                return
    ...
    def a_search_date(self, date, time):
        ...
        for activity in self.repo._activities:
            if re.search(date, activity.date, re.IGNORECASE) and \
                    activity.time[:activity.time.find('/')] <= time <= activity.time[activity.time.find('/') + 1:]:
                activities += str(activity) + '\n'
        if not activities:
            raise ValueError('      No activity was found.')
        return activities
    ...
    def a_sort_time(self):
        ...
        for act_nr in range(len(activities)):
            try:
                dates[str(activities[act_nr].date)] += int(
                    activities[act_nr].time[:activities[act_nr].time.find('/')]) - int(
                    activities[act_nr].time[activities[act_nr].time.find('/') + 1:])
            except:
                dates[str(activities[act_nr].date)] = int(
                    activities[act_nr].time[:activities[act_nr].time.find('/')]) - int(
                    activities[act_nr].time[activities[act_nr].time.find('/') + 1:])

        activities.sort(key=lambda activity: int(activity.date[:activity.date.find('/')]))
        activities.sort(key=lambda activity: int(activity.date[activity.date.find('/') + 1:activity.date.find('/', 3)]))
        activities.sort(key=lambda activity: int(activity.date[activity.date.find('/', 3) + 1:]))
    ...
    def a_person(self, id):
        ...
        for person in self.repo._persons:
            if person.pers_id == id:
                for activity in self.repo._activities:
                    if id in activity.act_pers_list:
                        activities += str(activity) + '\n'
                return activities
