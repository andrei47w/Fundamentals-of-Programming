...
def get_real(real_part):
    return real_part[0]

def get_imag(imag_part):
    return imag_part[1]

def get_length(clist):
    return len(clist)

def modulus(nr):
    return float(math.sqrt(get_real(complex_list[nr])**2 + get_imag(complex_list[nr])**2))
...
def search(op):
    ...
    last_pos=0
    max_count=0
    list_length=int(get_length(complex_list))
    if op==3:
        list_length-=1
...
def print_nr(nr):
    ...
    print(get_real(complex_list[nr]),"+", get_imag(complex_list[nr]), end="")
    print("*i", end="")
...
def option2():
    for i in range(0,get_length(complex_list)):
        print(i+1, ".  ", end="")
        print_nr(i)
        print()
